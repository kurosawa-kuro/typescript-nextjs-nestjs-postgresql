...
export const MicropostDetailModal: React.FC<MicropostDetailModalProps> = ({ post, onClose }) => {
  ...
  const avatarUrl = post.userAvatarPath ? `${API_BASE_URL}/uploads/${ImageUtils.normalizePath(post.userAvatarPath)}` : null;
          ...
          <div className="mb-4">
            ...
            <ul className="list-disc list-inside">
              {post.categories.map((category) => (
                <li key={category.id} className="text-black">{category.title}</li>
              ))}
            </ul>
          </div>
        )}
